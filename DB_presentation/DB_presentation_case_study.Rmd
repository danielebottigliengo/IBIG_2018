...
Before fitting the model to the real data, centering and scale 
...
- $13$ confounders: race, home oxygen use, current smoking status,
  ...
  chronic liver or kidney disease, asplenia, and other type of disease
...
```{r, fig.height = 6, fig.width = 10}
vaccine_eff_fun <- function(x) (1 - exp(x))
...
vaccine_eff_df <- map(
  ...
  setNames(
    object = ., nm = c(
      "uniform", "vague", "cauchy", "student_3", "student_7",
...
ggplot(
  ...
  geom_errorbarh(
    ...
    colour = "forestgreen", height = 0
  ) +
  geom_errorbarh(
    mapping = aes(xmin = lower_90, xmax = upper_90),
    size = 1, position = ggstance::position_dodgev(height = 0.4),
    height = 0, colour = "forestgreen"
  ) +
